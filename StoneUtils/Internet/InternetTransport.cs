...
namespace StoneUtils.Internet
{
    /// <summary>
    /// 网络传输类
    /// </summary>
    public class InternetTransport : ITransportFluxStore
    {
        ...
        {
            Console.WriteLine("**************************");
            Console.WriteLine(itemName + downloadFlux.ToString());
        }
        */
        /// <summary>
        /// 将记录保存到数据库当中，并初始化属性值
        ...
        {
            //TransportFluxRecordBLL bll = new TransportFluxRecordBLL(this);
            //bll.InsertRecord();
            string oldItemName = this.itemName;
            //[2012年2月21日]问题：Save的时候为什么要初始化，重新开始的时候应该显示的初始化，或者生成新对象，不然Save以后此对象的数据可能外界仍需要获取到
            //InitProperties();
